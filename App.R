...
# Check and install any missing packages automatically
required_pkgs <- c("shiny","shinydashboard","plotly","dplyr","readr","DT","tidyr",
                   "tseries","zoo","forecast","RcppRoll", "TTR")
...
# ---------------------------------------------
# User Interface: defines tabs, controls, and layout of the dashboard
# ---------------------------------------------
ui <- dashboardPage(
  ...
  dashboardBody(
    tabItems(
      # Dashboard tab: main overview with plots and anomaly stats
      ...
      # Statistical Tests tab: runs stationarity and normality checks
      ...
      # Compare & Correlate tab: compare two stocks and analyze correlation heatmap
      ...
      # Forecasting tab: short-term price prediction using ARIMA model
      ...
      # About tab: explains project background, objectives, and methods used
...
# ---------------------------------------------
# Server logic: data processing, analysis, and reactive outputs
# ---------------------------------------------
server <- function(input, output, session) {
  
  # Load the preprocessed dataset (generated by preprocess.R if missing)
  ...
  # Helper to ensure the data loads correctly as a tibble
  stock_data <- reactive({
    df <- stock_data_rdr()
  ...
  # Populate dropdown menus once the data is loaded
  observe({
    df <- stock_data()
    if (nrow(df) == 0) return()
  ...
  # Filter dataset based on selected company and date range
  ...
  # Summary boxes showing quick metrics for selected stock
  ...
  # Display a quick ADF test result for stationarity
  ...
  # Generate price chart, volatility chart, and anomaly table
  ...
  # Statistical Tests tab: deeper time-series diagnostics
  ...
  # Generate small summary of normality and stationarity tests
  ...
  # Create autocorrelation (ACF) bar plot for selected stock
  ...
  # Plot volume trend and highlight volume spikes
  ...
  # Add short explanatory notes for interpretation of results
  ...
  # Generate short written summary combining all statistical results
  ...
  # Compare & Correlate tab: analyze relationships between two or more stocks
  ...
  # Forecasting tab: build ARIMA model and visualize predicted prices
  
  # Update dropdown list once data is available
  observe({
    df <- stock_data()
    if (nrow(df) > 0) {
  ...
  forecast_results <- eventReactive(input$run_forecast, {
    ...
    # Show progress bar while ARIMA model fits and forecast runs
    withProgress(message = 'Generating forecast...', value = 0, {
      
      # Convert closing prices into a time series object
      ts_data <- ts(df$Close)
      
      # Fit ARIMA model (stepwise for faster computation)
      ...
      showNotification("Forecast generation complete!", type = "message", duration = 3)
      
      # Return both original data and forecast results
      list(data = df, forecast = fc)
    })
  })
  
  # Plot forecast results with 95% confidence interval
